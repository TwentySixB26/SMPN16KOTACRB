<header class="w-full items-center bg-white py-2 px-6 hidden sm:flex">
    <div class="w-1/2">
        <h1 class="text-base font-lexend font-medium text-gray-900">Hallo {{ auth()->user()->name }} 👋 </h1>
    </div>
    <div x-data="{ isOpen: false }" class="relative w-1/2 flex justify-end">
                <button @click="isOpen = !isOpen" class="realtive z-10 w-12 h-12 rounded-full overflow-hidden border-4 border-gray-400 hover:border-gray-300 focus:border-gray-300 focus:outline-none">
                    <img src="/storage/{{ auth()->user()->profile ?? 'profile/profile.png' }} " class="w-full h-full  object-cover">
                </button>
                <button x-show="isOpen" @click="isOpen = false" class="h-full w-full fixed inset-0 cursor-default"></button>
                <div x-show="isOpen" class="absolute w-32 bg-white rounded-lg shadow-lg py-2 mt-16 z-50">
                    <a href="/profileUser" class="block px-4 py-2 account-link hover:text-white">Profile</a>
                    <form action="/logout" method="post">
                        @csrf
                        <p class=" px-4 py-2 account-link hover:text-white">
                            <button type="submit" class="cursor-pointer text-left w-full ">
                                Sign Out
                            </button>
                        </p>
                    </form>
                </div>
    </div>
</header>
