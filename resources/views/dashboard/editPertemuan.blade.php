        ...
        <div class="w-full overflow-x-hidden border-t flex flex-col">
            <main class="w-full flex-grow p-6 mb-14">
                <div class="w-full border-t-2 border-t-blue-500 bg-gray-50 shadowMe py-5 sm:py-6 lg:py-4 px-3">
                    ...
                    <form action="/dashboard/{{ $pertemuan->id }}" method="post" class="mt-8">
                        @csrf
                        @method('put')
                        <div>
                            <h1 class=" font-acmeRegular font-semibold text-xl text-gray-700">Edit Deksripsi :</h1>
                            <textarea id="content"  name="content" class="mt-2 w-full h-[40px] sm:h-[40px]  lg:h-[40px] border-b border-slate-950  outline-none active:border-b text-xs sm:text-base lg:text-sm resize-none bg-transparent" placeholder="Tambahkan deksripsi" required maxlength="100">{{ $pertemuan->content }}</textarea>
